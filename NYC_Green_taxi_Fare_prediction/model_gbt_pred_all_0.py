...
gbt_model = GBTRegressionModel.load("./GBT_25iter_5depth-model_0")
gbt_predictions = gbt_model.transform(dataset_e)
gbt_predictions.select('prediction').show(5)
gbt_predictions.select(['i_total_amount','prediction']).toPandas().to_csv("./gbt_eval_all_pred_0.csv")
